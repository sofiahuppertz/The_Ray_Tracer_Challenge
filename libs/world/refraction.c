...
static t_color *compute_refracted_color(const t_world w, t_shape *shape, t_ray *refracted_ray, int remaining)
{
    t_color *final_color;
    t_color *temp_color;
...
t_color *refracted_color(const t_world w, const t_comps comps, int remaining)
{
    ...
    // Check if the material is transparent, if the recursion has reached its limit, or if the conditions for total internal reflection are met
