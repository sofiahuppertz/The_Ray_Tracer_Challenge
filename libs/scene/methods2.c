		...
		t_attributes *attributes, ...)
{
	...
	radius = diameter / 2.0;
	transform(&sp->shape, chain_tfs(translation(center->x, center->y,
				center->z), scaling(radius, radius, radius), NULL));
	if (attributes)
	{
...
void	make_cylinder(t_tuple *center, t_tuple *axis, double diameter,
		double height, t_color *color, t_attributes *attributes, ...)
{
	t_scene		*_scene;
	t_cylinder	*cy;
	t_matrix	*rotate;
...
void	make_cone(t_tuple *c, t_tuple *axis, double unit_d, double height,
		t_color *color, t_attributes *attributes, ...)
{
	t_scene		*_scene;
	t_matrix	*rotate;
	t_cone		*co;
