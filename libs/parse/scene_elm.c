...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/23 11:10:04 by lchiu             #+#    #+#             */
/*   Updated: 2024/05/23 11:10:04 by lchiu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_ambiant_light(t_parse *p, char *line)
{
	...
	make_ambient_light(ft_atof(split[1]), color(div_255(p->r), div_255(p->g),
			div_255(p->b)));
	free_split(split);
	return (0);
}
...
int	parse_camera(t_parse *p, char *line)
{
	...
	make_camera(point(p->x, p->y, p->z), point(p->vr_1, p->vr_2, p->vr_3),
		degrees_to_radians(ft_atoi(split[3])));
	free_split(split);
	return (0);
}
...
int	parse_light(t_parse *p, char *line)
{
	...
	make_light(point(p->x, p->y, p->z), ft_atof(split[2]), color(div_255(p->r),
			div_255(p->g), div_255(p->b)));
	free_split(split);
	return (0);
}
...
int	parse_sphere(t_parse *p, char *line)
{
	...
	{
		if (check_extra(split, p) == 1 || count_nb_elm(split, 4
				+ p->count) == 1)
		{
			free_split(split);
			return (1);
